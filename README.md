...
- `AUTH_MODE` — `legacy`, `oidc`, `hybrid`, or `email_otp` (Docker defaults to `oidc`)
...
- `SUPABASE_URL` — Supabase project URL used for passwordless email OTP
- `SUPABASE_ANON_KEY` — Supabase anonymous key used by the login page to request and verify OTP codes
- `EMAIL_OTP_ALLOWED_ADMIN_EMAILS` — comma-separated approved admin emails for OTP login
- `EMAIL_OTP_ALLOWED_ANALYST_EMAILS` — comma-separated approved analyst emails for OTP login
...
- In `AUTH_MODE=email_otp`, the login page sends a one-time code through Supabase Auth and the backend accepts only the configured analyst/admin emails.
...
- User creation/deletion is disabled in-app when `AUTH_MODE` uses external identity (`oidc` or `email_otp`) because account lifecycle is provider-managed.
