...
from app.auth_service import auth_mode_uses_email_code, auth_mode_uses_oidc, sync_user_from_claims
from app.config import AUTH_SESSION_COOKIE_NAME
from app.db import SessionLocal
from app.email_login import get_user_for_session
        ...
        async def wrapper(request, *args, **kwargs):
            ...
            if auth_mode_uses_email_code() and cookie_token:
                async with SessionLocal() as session:
                    user = await get_user_for_session(session, cookie_token)
                if not user:
